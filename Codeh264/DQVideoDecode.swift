...
class DQVideoDecode: NSObject {
    ...
    var decodeQueue = DispatchQueue(label: "decodeQueue")
    ...
    var videoDecodeCallBack: ((CVImageBuffer?)->())?
    func setVideoDecodeCallBack(block: ((CVImageBuffer?)->())?) {
    ...
    func initDecoder() -> Bool {
        ...
        return 状态
        */
        let descriptionState = CMVideoFormatDescriptionCreateFromH264ParameterSets(allocator: kCFAllocatorDefault, parameterSetCount: 2, parameterSetPointers: spsAndPps, parameterSetSizes: size, nalUnitHeaderLength: 4, formatDescriptionOut: &decodeDesc)
        ...
        * kCVPixelBufferWidthKey/kCVPixelBufferHeightKey: 视频源的分辨率 width*height
         ...
         */
        let imageBufferAttributes = [kCVPixelBufferOpenGLCompatibilityKey: kCVPixelFormatType_420YpCbCr8PlanarFullRange,
    ...
    private func setCallBack() {
        callBack = { decompressionOutputRefCon,
                     sourceFrameRefCon,
                     status,
                     inforFlags,
                     imageBuffer,
            ...
            let decoder = unsafeBitCast(decompressionOutputRefCon, to: DQVideoDecode.self)
    ...
    private func decode(frame:[UInt8],size:UInt32) {
        ...
        let decodeState = VTDecompressionSessionDecodeFrame(decompressionSession!, sampleBuffer: sampleBuffer!, flags: VTDecodeFrameFlags._EnableAsynchronousDecompression, frameRefcon: sourceFrame, infoFlagsOut: &inforFalg)
        if decodeState != 0 {
            print("解码失败")
        }
    }
